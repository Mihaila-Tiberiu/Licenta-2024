...
export default function MyBookingsPage() {
    ...
    async function handleDeleteBooking(IdRezervare) {
        try {
            await axios.put(`/cancelBookingByCustomer/${IdRezervare}`);

            const userEmailResponse = await axios.get(`/getHostEmailByBookingId/${IdRezervare}`);
            const bookingDetailsResponse = await axios.get(`/getBookingDetails/${IdRezervare}`);
    
            const emailParams = {
                subiect: 'Rezervare anulată de către oaspete',
                mesaj: `Rezervarea cu data de check-in ${bookingDetailsResponse.data.CheckInDate} și data de check-out ${bookingDetailsResponse.data.CheckOutDate} a fost anulată de către oaspete.`,
            ...
            Alert.showAlert('Rezervarea a fost anulată și gazda a fost notificată.');
        } catch (error) {
